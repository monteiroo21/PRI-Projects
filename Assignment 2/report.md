...
2.  **RAG as a Paradigm Shift:**
    The integration of **Retrieval-Augmented Generation (RAG)** fundamentally changed the system's utility. By injecting the **Top-5 reranked documents** into the **Gemini 2.5 Flash Lite** context, we transformed the user experience from simply "finding documents" to "getting answers." The engineering decision to use the Flash Lite model proved critical: it allowed us to maintain the large context window (1M tokens) required for multi-document reasoning while ensuring the high throughput (RPM) necessary for a responsive search interface.
